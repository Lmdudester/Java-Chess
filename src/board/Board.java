...
public class Board {
	...
	/**
	   ...
	   */
	public static char intToDigit(int y) {
		switch(y) {
			case 0:
				return '1';
			case 1:
	...
	/**
	   ...
	   */
	public static boolean inBounds(int x1, int y1, int x2, int y2) {
		if(x1 < 0 || x1 > 7)
			return false;
		if(x2 < 0 || x2 > 7)
			return false;
	...
	public boolean takeTurn(String s) {
		...
		if(s.length() == 7)
			promote = s.charAt(6);
		if(promote != 'Q' && promote != 'B' && promote != 'N' && promote != 'R')
			return false;
		
		// Make sure x's and y's are in bounds
		if(!Board.inBounds(x1, y1, x2, y2))
			return false;
